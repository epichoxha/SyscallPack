...
//redefine OBJECT_ATTRIBUTES struct
...
#else
    kernel32dll = __readfsdword( 0x30 );
#endif
...
UINT64 GetNtdll() {
...
#if _WIN64
    ntdll = __readgsqword( 0x60 );
#else
    ntdll = __readfsdword( 0x30 );
#endif

    ntdll = (ULONG_PTR)((_PPEB)ntdll)->pLdr;
	...
	while( val1 ) {
        ...
        //calculate the hash of ntdll.dll
		do {
			val3 = ror13( (DWORD)val3 );
			if( *((BYTE *)val2) >= 'a' )
				val3 += *((BYTE *)val2) - 0x20;
		...
		// compare the hash ntdll.dll
		if( (DWORD)val3 == NTDLLDLL_HASH ) {
            //return ntdll.dll if found
            ntdll = (ULONG_PTR)((PLDR_DATA_TABLE_ENTRY)val1)->DllBase;
            return ntdll;
        }
        val1 = DEREF( val1 );
    }
